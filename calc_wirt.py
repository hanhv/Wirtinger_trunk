...
def create_knot_dictionary(gauss_code):
    ...
    # Return the knot dictionary,
    # a dictionary in which each key is a strand label and each entry is a 2-element list,
    # the first element being the subset of the gauss
    # code defining the strand and the second being a list of the crossings for which the strand is the overstrand
    return knot_dict


# Function to find and label each strand in the knot diagram from the gauss code.
# Each strand is a sub-interval of the gauss code lying between two negative numbers
def find_strands(gauss_code_list):
    # Creates a set to store the strands
    strand_set = set()

    # Find the strands in each link in the list
    for x in range(len(gauss_code_list)):
        gauss_code = gauss_code_list[x]
        i = 0
        ...
        while True:
            ...
            if gauss_code[i] < 0:
                ...
                beginning = i
                # Iterate i mod len(gauss_code); that way, if i > len(gauss_code),
                # the program "wraps around" back to the first entry
                ...
                while gauss_code[i] > 0:
                    ...
                    i = (i + 1) % len(gauss_code)
                # If the beginning entry is later in the list than the next negative one
                # (e.g. the program has wrapped around back to the start of the list)...
                if beginning >= i:
    ...
    # Expanding strand labeling to work for knots up to 702 strands
    # by including labels of form 'AA', 'AB', etc, past A-Z:
    # If more strand labels are needed
    # (technically it's less than one strand per two crossings, but close enough)...
    if (len(gauss_code)) / 2 > 26:
        # Determine how many more strands are needed
        # (again, technically much less than this, but close enough.
        # This piece of code dosen't take that long to iterate)
    ...
    strands_dict = dict()
    # Enumerate retuns a list of two-tuples
    # where the first entry is an integer i running from 1 to len(strand_set) and the second is the ith entry
...
# Runs through each strand in strand_dict.
# At each one, runs through each positive number in its gauss code.
# Looks for the two strands that begin or end with the negative
# version of that number. These are the understands of that crossing.
# Once these have been found, add them to the list of crossings associated with that overstrand, and
...
def find_crossings(knot_dict, gauss_code):
    ...
    for key_outer in knot_dict:
        ...
        for under in knot_dict[key_outer][0]:
            # If the element is not negatve, this strand may be an overstrand of a new crossing,
            # so declare found1, found2 false to move onto next step
            if under > 0:
...
def wirt_main(gauss_code):
    ...
    seed_strand_set, wirt_num = calc_wirt_info(
        knot_dict)  # Determines the smallest length seed strand set and its length, which is w(d)
